...
class FunctionMinimizer(Annealer):
	...
	def update_mean(self):
		...
		channel_count = self.data.shape[2]
		cluster_number = self.cluster_number 
		self.mean = np.zeros((cluster_number,channel_count))
		count_sample = [0 for i in xrange(cluster_number)]
		std_dev = np.zeros((cluster_number,channel_count))
		for i in xrange(0,row_size):
			for j in xrange(0,col_size):
				self.mean[self.state[i][j]] = self.mean[self.state[i][j]] + self.data[i][j]
				count_sample[self.state[i][j]] += 1
		for i in xrange(0,cluster_number):
			self.mean[i] = self.mean[i]/count_sample[i]
	...
	def update_sigma(self):
		...
		cluster_number = self.cluster_number 
		count_sample = [0 for i in xrange(cluster_number)]
		self.std_dev = np.zeros((cluster_number,channel_count))
		for i in xrange(0,row_size):
			for j in xrange(0,col_size):
				self.std_dev[self.state[i][j]] = self.std_dev[self.state[i][j]] + ((self.data[i][j] - self.mean[self.state[i][j]])**2)
				count_sample[self.state[i][j]] += 1
		for i in xrange(0,cluster_number):
			self.std_dev[i] = (self.std_dev[i]/(count_sample[i]-1))**0.5
